...
Making of the new variable 'city\_state'
========================================

homicide\_citystate = homicide\_data %&gt;% mutate(city\_state = str\_c(city, ", " , state)) %&gt;% select(victim\_race, victim\_age, disposition, city\_state)

homicide\_summarize = homicide\_citystate %&gt;% filter(disposition %in% c("Closed without arrest", "Open/No arrest")) %&gt;% mutate (column = recode(disposition, `Closed without arrest` = "homicides", `Open/No arrest` = "unsolved\_homicides")) %&gt;% group\_by(city\_state, disposition) %&gt;% count()
