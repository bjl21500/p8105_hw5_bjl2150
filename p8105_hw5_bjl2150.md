...
city\_wide\_prop = map(df*t**o**t**a**l*<sub>*h*</sub>*o**m**i**c**i**d**e**s*, *d**f*unsolved\_homicides, prop.test)

map2(.x, .y, .f, ...)

output = homicide\_summarize %&gt;% filter(total\_homicides != "NA") %&gt;% mutate(new = map2(.x = unsolved\_homicides, .y = total\_homicides, ~prop.test(x = .x, n = .y))) %&gt;% mutate(new =map(new, broom::tidy)) %&gt;% unnest()

city\_state\_output = output %&gt;% select(city\_state, estimate, conf.low, conf.high, p.value)

city\_state\_output ggplot(weather\_df, aes(x = name, y = tmax)) + geom\_boxplot()

\`\`\`

map2(df*S**u**c**c**e**s**s*, *d**f*N, prop.test)

map(weather\_nest$data, ~lm(tmax ~ tmin, data = .x))

output = map\_df(.x = df, ~ mean\_and\_sd(.x))
