...
``` r
# Usng the prrop.test within a mutate function in order to create a new variable where the map function was appplied to. Then tidied and unnested the 'new' column
output = homicide_summarize %>%
...
# Specifically selelcted out the estimates and both confidence intervals per city, state. Additonally, reordering the 'estimate' variable.
city_state_output = output %>%
  select(city_state, estimate, conf.low, conf.high, p.value) %>%
  mutate(city_state = forcats::fct_reorder(city_state, estimate))
```

city\_state\_output %&gt;% ggplot(aes(x = city\_state, y = estimate)) + geom\_boxplot() + theme(axis.text.x = element\_text(angle = 90, hjust = 1))
